...
export interface ReasoningRetrievalInput { scope: string; query: string; /** Explicit host-provided conditions are hard applicability gates. */ taskType?: string; riskLevel?: "low" | "medium" | "high"; environment?: string; availableTools?: string[]; /** Runtime classification is advisory: it ranks candidates but never excludes them. */ inferredTaskType?: string; inferredRiskLevel?: "low" | "medium" | "high"; /** Preserves conservative quality thresholds without turning an inference into an applicability gate. */ qualityRiskLevel?: "low" | "medium" | "high"; limit?: number; semanticScores?: Readonly<Record<string, number>>; qualityPolicy?: ReasoningQualityPolicy; now?: () => number; }
...
export class ReasoningRetrievalService {
  ...
  private applicability(value: ReasoningApplicability, context: ReturnType<typeof normalizeContext>, excluded: ReasoningRetrievalResult["excluded"], reasons: string[]): number | undefined {
    ...
    const inferred = (field: string[], requested: string | undefined, label: "task_type" | "risk_level") => {
      if (!requested) return;
      checks++;
      if (!field.length) { score += .5; reasons.push(`${label}_inferred_generic`); return; }
      if (field.includes(requested)) { score++; reasons.push(`${label}_inferred_match`); return; }
      reasons.push(`${label}_inferred_mismatch`);
    };
    inferred(value.taskTypes, context.inferredTaskType, "task_type");
    inferred(value.riskLevels, context.inferredRiskLevel, "risk_level");
...
function normalizeContext(input: ReasoningRetrievalInput) {
  const risk = riskLevel(input.riskLevel), inferredRiskLevel = riskLevel(input.inferredRiskLevel), qualityRiskLevel = riskLevel(input.qualityRiskLevel) ?? risk ?? inferredRiskLevel;
  const taskType = reasoningTaskType(input.taskType), availableTools = input.availableTools === undefined ? undefined : [...new Set(input.availableTools.map(identifier).filter((value): value is string => Boolean(value)))].slice(0, 20);
  return { taskType, riskLevel: risk, environment: identifier(input.environment), availableTools, inferredTaskType: taskType ? undefined : reasoningTaskType(input.inferredTaskType), inferredRiskLevel: risk ? undefined : inferredRiskLevel, qualityRiskLevel };
}
...
function riskLevel(value: unknown): "low" | "medium" | "high" | undefined { if (value === undefined) return undefined; if (["low", "medium", "high"].includes(String(value))) return value as "low" | "medium" | "high"; throw new Error("invalid_reasoning_retrieval"); }
