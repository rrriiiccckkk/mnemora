...
export class DecisionMemoryService {
  ...
  get(id: string, scope: string): DecisionMemory | undefined {
    const safe = normalizeScope(scope), row = this.db.prepare("SELECT * FROM mnemora_decisions WHERE id=? AND scope=?").get(id, safe) as Record<string, unknown> | undefined;
    if (!row) return;
    const evidence = (this.db.prepare("SELECT source_ref,relation FROM mnemora_decision_evidence WHERE decision_id=? ORDER BY source_ref,relation").all(id) as Array<{ source_ref: string; relation: EvidenceRelation }>).map(item => ({ sourceRef: item.source_ref, relation: item.relation }));
    const episodeIds = (this.db.prepare("SELECT episode_id FROM mnemora_decision_episodes WHERE decision_id=? ORDER BY episode_id").all(id) as Array<{ episode_id: string }>).map(item => item.episode_id);
    const review = this.db.prepare("SELECT 1 FROM mnemora_decision_evidence_reviews WHERE decision_id=? AND scope=? AND status='needs_review'").get(id, safe);
    return {
      id: String(row.id), scope: String(row.scope), objective: String(row.objective),
      ...(row.scenario ? { scenario: String(row.scenario) } : {}), alternatives: array(row.alternatives_json),
      ...(row.chosen_action ? { chosenAction: String(row.chosen_action) } : {}), ...(row.outcome ? { outcome: String(row.outcome) } : {}), ...(row.rationale ? { rationale: String(row.rationale) } : {}),
      constraints: array(row.constraints_json), ...(row.confidence !== null ? { confidence: Number(row.confidence) } : {}), decisionMaker: row.decision_maker as DecisionMaker,
      ...(row.decided_at !== null ? { decidedAt: Number(row.decided_at) } : {}), ...(row.valid_from !== null ? { validFrom: Number(row.valid_from) } : {}), ...(row.valid_until !== null ? { validUntil: Number(row.valid_until) } : {}),
      recordedAt: Number(row.recorded_at), ...(row.superseded_at ? { supersededAt: Number(row.superseded_at) } : {}), ...(row.invalidated_at ? { invalidatedAt: Number(row.invalidated_at) } : {}), ...(row.previous_version_id ? { previousVersionId: String(row.previous_version_id) } : {}),
      status: review && row.status === "active" ? "needs_review" : row.status as DecisionStatus, evidence, episodeIds
    };
  }
