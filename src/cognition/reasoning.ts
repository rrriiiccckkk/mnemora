...
export type ReasoningVerificationAssertion = { kind: "tool_result"; tool: string; expected: "success" | "failure" } | { kind: "task_outcome"; expected: "success" | "failure" | "partial" } | { kind: "strategy_adoption"; expected: true };
/** A bounded, deterministic verification contract. It stores no tool payloads, prompts, model output, or session data. */
export interface ReasoningVerificationSpec { version: "reasoning-verification-v1"; assertions: ReasoningVerificationAssertion[]; }
export interface ReasoningMemoryInput { scope: string; kind: ReasoningMemoryKind; strategy: string; applicability?: Partial<ReasoningApplicability>; contraindications?: string[]; sourceTaskRefs: string[]; outcomeRefs: string[]; evidenceRefs: string[]; verification?: ReasoningVerificationSpec; confidence?: number; supersedesId?: string; }
export interface ReasoningMemory { id: string; scope: string; kind: ReasoningMemoryKind; strategy: string; applicability: ReasoningApplicability; contraindications: string[]; sourceTaskRefs: string[]; outcomeRefs: string[]; evidenceRefs: string[]; verification?: ReasoningVerificationSpec; confidence: number; utilityScore: number; successCount: number; failureCount: number; degradedCount: number; state: ReasoningMemoryState; supersedesId?: string; createdAt: number; updatedAt: number; }
...
export class ReasoningMemoryService {
  ...
  private normalize(input: ReasoningMemoryInput) {
    ...
    const utility = this.utility(outcomeRefs, scope), verification = verificationSpec(input.verification), supersedesId = clean(input.supersedesId, 256);
    return { scope, kind, strategy, applicability, contraindications, sourceTaskRefs, outcomeRefs, evidenceRefs, ...(verification ? { verification } : {}), confidence, ...utility, supersedesId };
  }
  ...
  private read(row: Record<string, unknown>): ReasoningMemory { const verification = verificationSpec(parseJson(row.verification_json)); return { id: String(row.id), scope: String(row.scope), kind: row.kind as ReasoningMemoryKind, strategy: String(row.strategy), applicability: parseApplicability(row.applicability_json), contraindications: strings(row.contraindications_json), sourceTaskRefs: strings(row.source_task_refs_json), outcomeRefs: strings(row.outcome_refs_json), evidenceRefs: strings(row.evidence_refs_json), ...(verification ? { verification } : {}), confidence: Number(row.confidence), utilityScore: Number(row.utility_score), successCount: Number(row.success_count), failureCount: Number(row.failure_count), degradedCount: Number(row.degraded_count), state: row.state as ReasoningMemoryState, ...(row.supersedes_id ? { supersedesId: String(row.supersedes_id) } : {}), createdAt: Number(row.created_at), updatedAt: Number(row.updated_at) }; }
}
const parseJson = (value: unknown): unknown => { if (typeof value !== "string" || !value) return undefined; try { return JSON.parse(value); } catch { throw new Error("invalid_reasoning_verification"); } };
...
const verificationSpec = (value: unknown): ReasoningVerificationSpec | undefined => {
  if (value === undefined || value === null) return undefined;
  const input = record(value);
  if (!input || !exactKeys(input, ["version", "assertions"]) || input.version !== "reasoning-verification-v1" || !Array.isArray(input.assertions) || !input.assertions.length || input.assertions.length > 16) throw new Error("invalid_reasoning_verification");
  const assertions = input.assertions.map((item): ReasoningVerificationAssertion => {
    const assertion = record(item);
    if (!assertion || typeof assertion.kind !== "string") throw new Error("invalid_reasoning_verification");
    if (assertion.kind === "tool_result") {
      const rawTool = typeof assertion.tool === "string" ? assertion.tool.trim() : undefined, tool = rawTool?.toLowerCase();
      if (!exactKeys(assertion, ["kind", "tool", "expected"]) || !tool || tool.length > 80 || !/^[a-z0-9][a-z0-9:_-]{0,79}$/.test(tool) || (assertion.expected !== "success" && assertion.expected !== "failure")) throw new Error("invalid_reasoning_verification");
      return { kind: "tool_result", tool, expected: assertion.expected };
    }
    if (assertion.kind === "task_outcome" && exactKeys(assertion, ["kind", "expected"]) && (assertion.expected === "success" || assertion.expected === "failure" || assertion.expected === "partial")) return { kind: "task_outcome", expected: assertion.expected };
    if (assertion.kind === "strategy_adoption" && exactKeys(assertion, ["kind", "expected"]) && assertion.expected === true) return { kind: "strategy_adoption", expected: true };
    throw new Error("invalid_reasoning_verification");
  });
  return { version: "reasoning-verification-v1", assertions };
};
