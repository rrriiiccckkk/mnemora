...
export interface ReasoningDeliveryConfig { enabled: boolean; scopes: string[]; adapter: "openclaw"; calibrationMaxAgeHours: number; maxConsecutiveDeliveries: number; }
...
export interface ReasoningDeliveryResult { appendSystemContext: string; }
...
export class ReasoningRuntimeGovernanceRepository {
  ...
  recordDelivery(input: { scope: string; calibrationId: string; status: "delivered" | "withheld" | "failed"; selectedCount: number; estimatedTokens: number; durationMs: number; reasonCode: "delivered" | "no_trigger" | "empty" | "budget" | "cadence" | "operation_failed" }): string {
    const id = `reasoning-delivery:${randomUUID()}`; this.db.prepare("INSERT INTO mnemora_reasoning_runtime_delivery_runs(id,scope,calibration_id,status,selected_count,estimated_tokens,duration_ms,reason_code,feedback,created_at) VALUES(?,?,?,?,?,?,?,?, 'unknown',?)").run(id, normalizeScope(input.scope), input.calibrationId, input.status, integer(input.selectedCount, 20), integer(input.estimatedTokens, 1600), integer(input.durationMs, 30000), input.reasonCode, this.now()); return id;
  }
...
export class ReasoningGovernedDeliveryService {
  private readonly shadow: ReasoningRuntimeShadowService; private readonly governance: ReasoningRuntimeGovernanceRepository;
  constructor(db: DatabaseSyncInstance, private readonly config: ReasoningRuntimeGovernanceConfig, now: () => number = Date.now) { this.shadow = new ReasoningRuntimeShadowService(db, config, now); this.governance = new ReasoningRuntimeGovernanceRepository(db, now); }
  handle(input: ReasoningRuntimeTaskContext, options: { deliveryAllowed?: boolean } = {}): ReasoningDeliveryResult | undefined {
    ...
    const presentation = fitPresentation(result.context, this.config.tokenBudget, this.config.delivery.adapter); if (!presentation) { this.governance.recordDelivery({ scope: input.scope, calibrationId: authorization.calibration.id, status: "withheld", selectedCount: 0, estimatedTokens: 0, durationMs: Date.now() - started, reasonCode: "budget" }); return undefined; }
    this.governance.recordDelivery({ scope: input.scope, calibrationId: authorization.calibration.id, status: "delivered", selectedCount: presentation.items, estimatedTokens: presentation.tokens, durationMs: Date.now() - started, reasonCode: "delivered" }); return { appendSystemContext: presentation.content };
  }
}
