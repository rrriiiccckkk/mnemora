...
export function createOpenClawToolDefinitions(openGraph: () => Mnemora, surface: ToolSurface = "full"): OpenClawToolDefinition[] {
  ...
  const definitions = [
    ...
    descriptor("kg_review", "KG Review", scoped("Review duplicate candidates, relationship anomalies, schema-drift audits, semantic-pattern proposals, evidence-backed related-edge proposals, a bounded read-only remediation worklist, or graph hygiene. Worklists can record only a stale-candidate overlay; graph changes remain preview then explicit confirm."), Type.Object({ kind: Type.Optional(Type.Union([Type.Literal("duplicates"), Type.Literal("anomalies"), Type.Literal("identity"), Type.Literal("schema_drift"), Type.Literal("semantic_patterns"), Type.Literal("related_edge_refinements"), Type.Literal("related_edge_semantics"), Type.Literal("hygiene"), Type.Literal("worklist")])), status: Type.Optional(Type.Union([Type.Literal("pending"), Type.Literal("ignored"), Type.Literal("rejected"), Type.Literal("merged"), Type.Literal("invalidated")])), scan: Type.Optional(Type.Boolean()), limit: Type.Optional(Type.Number({ minimum: 1, maximum: 100 })), after_id: Type.Optional(Type.String()), candidate_id: Type.Optional(Type.String()), decision: Type.Optional(Type.Union([Type.Literal("ignored"), Type.Literal("rejected"), Type.Literal("accepted")])), repair_type: Type.Optional(Type.Union([Type.Literal("depends_on"), Type.Literal("part_of"), Type.Literal("instance_of"), Type.Literal("related_to")])), preview_hash: Type.Optional(Type.String({ minLength: 64, maxLength: 64 })), confirm: Type.Optional(Type.Boolean()), scope: Type.Optional(scopeSchema), approval_id: Type.Optional(Type.String({ minLength: 1, maxLength: 200 })) }, { additionalProperties: false }),
      (graph, p) => {
...
const optionalReviewKind = (value: unknown): "duplicates" | "anomalies" | "identity" | "schema_drift" | "semantic_patterns" | "related_edge_refinements" | "related_edge_semantics" | "hygiene" | "worklist" => value === "anomalies" || value === "identity" || value === "schema_drift" || value === "semantic_patterns" || value === "related_edge_refinements" || value === "related_edge_semantics" || value === "hygiene" || value === "worklist" ? value : "duplicates";
const optionalReviewStatus = (value: unknown): import("./tools.js").ReviewStatus => value === "invalidated" || value === "ignored" || value === "rejected" || value === "merged" ? value : "pending";
