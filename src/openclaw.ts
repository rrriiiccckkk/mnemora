...
export function createOpenClawToolDefinitions(openGraph: () => Mnemora, surface: ToolSurface = "full"): OpenClawToolDefinition[] {
  ...
  const definitions = [
    ...
    descriptor("kg_review", "KG Review", scoped("Review duplicate candidates, relationship anomalies, schema-drift audits, semantic-pattern proposals, evidence-backed related-edge refinements, source-backed related-edge semantic labels, or read-only graph hygiene. Related-edge reviews are scan, preview, then explicit confirm; they never automatically change graph evidence or topology."), Type.Object({ kind: Type.Optional(Type.Union([Type.Literal("duplicates"), Type.Literal("anomalies"), Type.Literal("identity"), Type.Literal("schema_drift"), Type.Literal("semantic_patterns"), Type.Literal("related_edge_refinements"), Type.Literal("related_edge_semantics"), Type.Literal("hygiene")])), status: Type.Optional(Type.Union([Type.Literal("pending"), Type.Literal("ignored"), Type.Literal("rejected"), Type.Literal("merged")])), scan: Type.Optional(Type.Boolean()), limit: Type.Optional(Type.Number({ minimum: 1, maximum: 100 })), after_id: Type.Optional(Type.String()), candidate_id: Type.Optional(Type.String()), decision: Type.Optional(Type.Union([Type.Literal("ignored"), Type.Literal("rejected"), Type.Literal("accepted")])), repair_type: Type.Optional(Type.Union([Type.Literal("depends_on"), Type.Literal("part_of"), Type.Literal("instance_of"), Type.Literal("related_to")])), preview_hash: Type.Optional(Type.String({ minLength: 64, maxLength: 64 })), confirm: Type.Optional(Type.Boolean()), scope: Type.Optional(scopeSchema), approval_id: Type.Optional(Type.String({ minLength: 1, maxLength: 200 })) }, { additionalProperties: false }),
      (graph, p) => {
