...
export function createOpenClawToolDefinitions(openGraph: () => Mnemora, surface: ToolSurface = "full"): OpenClawToolDefinition[] {
  ...
  const definitions = [
    ...
    descriptor("kg_related", "KG Related", scoped("Traverse stable graph topology from an entity with scope-filtered evidence. Request a built-in edge type or an exact operator-approved semantic predicate explicitly; labels never expand traversal."), Type.Object({ entity: Type.String({ description: "Entity id, slug, alias, or search text." }), depth: Type.Optional(Type.Number({ description: "Traversal depth." })), edge_types: Type.Optional(Type.Array(Type.String({ description: "Structural relationship filter, or built-in semantic-label predicate." }))), semantic_predicates: Type.Optional(Type.Array(Type.String({ minLength: 1, maxLength: 64, description: "Exact operator-approved semantic labels to inspect without changing topology." }), { maxItems: 12 })), direction: Type.Optional(Type.Union([Type.Literal("out"), Type.Literal("in"), Type.Literal("both")])), scope: Type.Optional(scopeSchema) }, { additionalProperties: false }),
      (graph, p) => graph.kg_related(String(p.entity ?? ""), optionalNumber(p.depth) ?? 1, optionalStringArray(p.edge_types), optionalDirection(p.direction), optionalString(p.scope), optionalSemanticPredicates(p.semantic_predicates))),
    ...
    descriptor("kg_review", "KG Review", scoped("Review duplicate candidates, relationship anomalies, schema-drift audits, semantic-pattern and vocabulary proposals, evidence-backed related-edge proposals, a bounded read-only remediation worklist, or graph hygiene. Schema-drift repair or rejection remains preview then explicit confirm; neither path automatically retypes an entity."), Type.Object({ kind: Type.Optional(Type.Union([Type.Literal("duplicates"), Type.Literal("anomalies"), Type.Literal("identity"), Type.Literal("schema_drift"), Type.Literal("semantic_patterns"), Type.Literal("semantic_vocabulary"), Type.Literal("related_edge_refinements"), Type.Literal("related_edge_semantics"), Type.Literal("hygiene"), Type.Literal("worklist")])), status: Type.Optional(Type.Union([Type.Literal("pending"), Type.Literal("accepted"), Type.Literal("ignored"), Type.Literal("rejected"), Type.Literal("merged"), Type.Literal("invalidated")])), scan: Type.Optional(Type.Boolean()), limit: Type.Optional(Type.Number({ minimum: 1, maximum: 100 })), after_id: Type.Optional(Type.String()), candidate_id: Type.Optional(Type.String()), decision: Type.Optional(Type.Union([Type.Literal("ignored"), Type.Literal("rejected"), Type.Literal("accepted")])), repair_type: Type.Optional(Type.Union([Type.Literal("depends_on"), Type.Literal("part_of"), Type.Literal("instance_of"), Type.Literal("related_to")])), preview_hash: Type.Optional(Type.String({ minLength: 64, maxLength: 64 })), confirm: Type.Optional(Type.Boolean()), scope: Type.Optional(scopeSchema), approval_id: Type.Optional(Type.String({ minLength: 1, maxLength: 200 })) }, { additionalProperties: false }),
      (graph, p) => {
        ...
        const kind = optionalReviewKind(p.kind), status = optionalReviewStatus(p.status), scan = p.scan === true, limit = optionalNumber(p.limit) ?? 20, afterId = optionalString(p.after_id), scope = optionalString(p.scope), approvalId = optionalString(p.approval_id);
    ...
    descriptor("kg_recall_metrics", "KG Recall Metrics", scoped("List bounded, redacted adaptive and automatic-recall telemetry for one scope. It never stores prompt text, candidate content, identifiers, or sources, and never changes retrieval or injected context."), Type.Object({ scope: Type.Optional(scopeSchema), limit: Type.Optional(Type.Integer({ minimum: 1, maximum: 100 })) }, { additionalProperties: false }), (graph, p) => graph.kg_recall_metrics({ scope: optionalString(p.scope), limit: optionalNumber(p.limit) }), true),
...
const optionalStringArray = (value: unknown): RelationshipType[] | undefined => Array.isArray(value)
  ...
  : undefined;
const optionalSemanticPredicates = (value: unknown): string[] | undefined => Array.isArray(value)
  ? [...new Set(value.filter((item): item is string => typeof item === "string" && /^[a-z][a-z0-9_]{0,63}$/.test(item)).map(item => item.toLowerCase()))].slice(0, 12)
  : undefined;
...
const optionalReviewKind = (value: unknown): "duplicates" | "anomalies" | "identity" | "schema_drift" | "semantic_patterns" | "semantic_vocabulary" | "related_edge_refinements" | "related_edge_semantics" | "hygiene" | "worklist" => value === "anomalies" || value === "identity" || value === "schema_drift" || value === "semantic_patterns" || value === "semantic_vocabulary" || value === "related_edge_refinements" || value === "related_edge_semantics" || value === "hygiene" || value === "worklist" ? value : "duplicates";
const optionalReviewStatus = (value: unknown): import("./tools.js").ReviewStatus => value === "accepted" || value === "invalidated" || value === "ignored" || value === "rejected" || value === "merged" ? value : "pending";
