...
function firstUseOutcome(runtime: PluginRuntime, graph: ReturnType<PluginRuntime["openGraph"]>): MnemoraOperatorOutcome {
  ...
  const result = firstUseVerification({
    ...
    recall: { totalRuns: recall.total_runs, attachedRuns: recall.attached_runs }
  });
...
function help(): MnemoraOperatorOutcome { return { level: "warning", message: "Mnemora: use /mnemora status, /mnemora verify, /mnemora doctor, /mnemora corpus sync, or /mnemora corpus search <query>." }; }
