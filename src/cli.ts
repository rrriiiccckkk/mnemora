...
function cognitionCommand(graph: Mnemora, raw: string[]): unknown {
  ...
  if (family === "reasoning") {
    ...
    if (operation === "runtime-canary-status") { requireNone(positional); const config = reasoningGovernanceConfig(graph); return new ReasoningRuntimeGovernanceRepository(graph.store.db).status(scope, config); }
    if (operation === "runtime-canary-enable") { const calibrationId = requiredArgument(positional, "calibration_id"), config = reasoningGovernanceConfig(graph), governance = new ReasoningRuntimeGovernanceRepository(graph.store.db), preview = governance.enablePreview(scope, calibrationId, config); if (options.confirm !== true) return preview; return governance.enable(scope, calibrationId, config, option(options, "preview-hash") ?? ""); }
