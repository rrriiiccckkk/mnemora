...
export function createInspectorApplication(options: { graph: Mnemora; allowOperations: boolean; artifactDirectory: string; now?: () => number; randomBytes?: (size: number) => Buffer }): InspectorApplication {
  ...
  return {
    ...base,
    capabilities: () => ({ operations: true, graph_revision: options.graph.store.graphRevision(), config_revision: options.graph.store.sourceTrustRevision() }),
    operationPreview: input => dispatch(input, "preview"), operationConfirm: input => dispatch(input, "confirm"),
...
function memoryCorrectionPreview(input: unknown, defaultScope: string | undefined): { scope?: string; kind: MemoryCorrectionKind; id: string } {
  if (typeof input !== "object" || input === null || Array.isArray(input)) throw new Error("invalid_memory_correction");
  const value = input as Record<string, unknown>, allowed = new Set(["scope", "kind", "id"]);
  if (!Object.keys(value).every(key => allowed.has(key)) || typeof value.kind !== "string" || !["event", "artifact", "episode", "summary"].includes(value.kind) || typeof value.id !== "string" || value.id.length === 0 || value.id.length > 512 || /[\u0000-\u001f]/.test(value.id) || value.scope !== undefined && (typeof value.scope !== "string" || !/^[a-z0-9][a-z0-9._:-]{0,79}$/.test(value.scope))) throw new Error("invalid_memory_correction");
  return { ...(typeof value.scope === "string" ? { scope: value.scope } : { scope: defaultScope }), kind: value.kind as MemoryCorrectionKind, id: value.id };
}

function memoryCorrectionConfirm(input: unknown): { preview_token: string } {
  if (typeof input !== "object" || input === null || Array.isArray(input)) throw new Error("invalid_memory_correction");
  const value = input as Record<string, unknown>;
  if (Object.keys(value).length !== 1 || typeof value.preview_token !== "string" || !/^memory-correction:[A-Za-z0-9_-]{43}$/.test(value.preview_token)) throw new Error("invalid_memory_correction");
  return { preview_token: value.preview_token };
}
...
function intelligenceView(graph: Mnemora, intelligence: MemoryIntelligenceService, input: unknown): unknown | Promise<unknown> {
  ...
  return graph.kg_recall_explain({ query, scope, max_nodes: 10, max_depth: 1, token_budget: 800, mode: "lexical" }).then(result => ({
    ...
    injected: { candidates_considered: result.candidates.length, nodes: result.candidates.filter(item => item.decision === "included").length, memories: result.memories.length, budget_tokens: 800 }, actual_attachment: actualAttachment(graph, scope, query), truncated: result.truncated, mode: "lexical"
  })).catch(() => ({ kind: "memory_intelligence", view: "retrieval", scope, items: [{ status: "unavailable" }], truncated: false }));
}

/** Actual attachment is reported only from the redacted ContextEngine telemetry.
 * A routed query is deliberately not compared with this direct policy trace. */
function actualAttachment(graph: Mnemora, scope: string, query: string) {
  if (graph.config.unifiedRetrieval?.shadowMode !== true) return { telemetry: "disabled" as const, status: "not_observed" as const };
  const planned = planRecallQuery(query, graph.config.recall?.queryRouting);
  if (planned.query !== query) return { telemetry: "available" as const, status: "not_comparable" as const };
  const queryHash = createHash("sha256").update(planned.query).digest("hex");
  const run = graph.unifiedRecallShadow.list(scope, 100).items.find(item => item.query_hash === queryHash);
  if (!run) return { telemetry: "available" as const, status: "not_observed" as const };
  return { telemetry: "available" as const, status: "matched" as const, attached: run.attached, created_at: run.created_at, local_selected_count: run.local_selected_count, graph_attached: run.graph_attached };
}
