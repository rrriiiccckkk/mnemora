...
test("automatic features are opt-in with bounded defaults", () => {
  ...
  assert.deepEqual(config.cognition, { formationShadow: true, admission: { mode: "shadow", preAdmission: { mode: "off" } }, beliefs: { enabled: false, autoCorroborate: false }, contextCompiler: { enabled: false, tokenBudget: 600, maxItems: 8 }, reflection: { enabled: false, maxJobsPerRun: 4, staleAfterDays: 90 }, graduation: { enabled: false }, reasoningRuntime: { shadowMode: false, scopes: [], tokenBudget: 800, maxItems: 6, minConfidence: .6, highRiskMinConfidence: .8, minEvidenceQuality: .5, highRiskMinEvidenceQuality: .75, maxStalenessDays: 365, excludeConflicted: true, retentionDays: 30, readiness: { minimumRuns: 25, maxErrorRate: .05, maxEmptyRate: .8, maxP95Ms: 100 }, delivery: { enabled: false, scopes: [], adapter: "openclaw", calibrationMaxAgeHours: 168, maxConsecutiveDeliveries: 2 } } });
...
test("reasoning runtime shadow remains off and clamps scope, quality, retention, and readiness policy", () => {
  const value = normalizeConfig({ cognition: { reasoningRuntime: { shadowMode: true, scopes: ["project:A", "bad scope", "project:a"], tokenBudget: 1, maxItems: 99, minConfidence: -1, highRiskMinConfidence: 2, minEvidenceQuality: -1, highRiskMinEvidenceQuality: 2, maxStalenessDays: 0, retentionDays: 999, readiness: { minimumRuns: 0, maxErrorRate: 2, maxEmptyRate: -1, maxP95Ms: 0 } } } }).cognition.reasoningRuntime;
  assert.deepEqual(value, { shadowMode: true, scopes: ["project:a"], tokenBudget: 64, maxItems: 12, minConfidence: 0, highRiskMinConfidence: 1, minEvidenceQuality: 0, highRiskMinEvidenceQuality: 1, maxStalenessDays: 1, excludeConflicted: true, retentionDays: 365, readiness: { minimumRuns: 1, maxErrorRate: 1, maxEmptyRate: 0, maxP95Ms: 1 }, delivery: { enabled: false, scopes: [], adapter: "openclaw", calibrationMaxAgeHours: 168, maxConsecutiveDeliveries: 2 } });
});

test("governed reasoning delivery requires explicit exact scopes and clamps canary controls", () => {
  const value = normalizeConfig({ cognition: { reasoningRuntime: { delivery: { enabled: true, scopes: ["project:A", "bad scope", "project:a"], adapter: "unsafe", calibrationMaxAgeHours: 0, maxConsecutiveDeliveries: 99 } } } }).cognition.reasoningRuntime.delivery;
  assert.deepEqual(value, { enabled: true, scopes: ["project:a"], adapter: "openclaw", calibrationMaxAgeHours: 1, maxConsecutiveDeliveries: 10 });
});
