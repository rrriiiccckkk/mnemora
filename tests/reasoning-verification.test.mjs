...
test("task outcomes enqueue deterministic verification, and only a bounded processor can open a mismatch circuit", () => {
  ...
  try {
    ...
    clock += 100;
    assert.deepEqual(verification.run({ scope: state.scope, limit: 10 }), { processed: 2, matched: 1, mismatched: 1, circuitsOpened: 1 });
    ...
    assert.deepEqual({ reason: feedback.circuit(state.scope, state.memory.id)?.reason, open: feedback.circuit(state.scope, state.memory.id)?.open, rerun: verification.run({ scope: state.scope, limit: 10 }) }, { reason: "verification_mismatch", open: true, rerun: { processed: 0, matched: 0, mismatched: 0, circuitsOpened: 0 } });
  } finally { store.close(); }
});
...
test("tool verification accepts only the contracted identifier, has no raw output channel, and is idempotent", () => {
  ...
  try {
    ...
    assert.deepEqual(verification.run({ scope: state.scope, limit: 1 }), { processed: 1, matched: 0, mismatched: 1, circuitsOpened: 1 });
...
test("schema v66 preserves every v65 circuit field while widening its reason contract and adding the local ledger", () => {
  ...
  try {
    ...
    try {
      ...
      assert.equal(SUPPORTED_SCHEMA_VERSION, 66);
      assert.equal(migrated.db.prepare("PRAGMA user_version").get().user_version, 66);
