...
运行 `npm run benchmark:task-resume` 可执行 26 条合成多会话功能序列。`mnemora evaluate task-resume-comparison <plan.json>` 只验证并报告三组对照实验契约，不会调用模型；内置计划会明确显示为 `real_effect_experiment_not_run`，直到获得授权的去标识化测量结果。已测量的结果必须逐条满足计划中的 token 和延迟预算。
